...
r = [0.03; 5000*2*pi];
x0 = [0.1;1000*2*pi];
%r = [0.04*1e2; 5000*2*pi*1e-3];
%x0 = [0.1*1e2;1000*2*pi*1e-3];
...
%%% Cost Function
%Q = [100 0; 0 1e-8]; % weights on width and freq deviation from reference
Q = 200;
S = 0.0001;
...
K=inv([A0-eye(2) B0; D 0])*[C; r(1)];
Ry = repmat(K(1),N,1); %output reference set
Ru = repmat(K(3),N,1); %input reference set

G = 2*(Psi + Gamma'*Xi'*Omega*Xi*Gamma);                % quadratic part of cost function (U^T G U)
F = 2*Gamma'*Xi'*Omega*(Xi*Phi*x0 + Xi*Lambda - Ry) - 2*Psi*Ru; % linear part of cost function (F^T U)
...
%             end
            ...
            G = 2*(Psi + Gamma'*Xi'*Omega*Xi*Gamma);                % update quadratic part of cost function (U^T G U)
            F = 2*Gamma'*Xi'*Omega*(Xi*Phi*x0 + Xi*Lambda - Ry) - 2*Psi*Ru; % update linear part of cost function (F^T U)
